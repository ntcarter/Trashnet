...
while True:
   ...
   if(lastDistance-distance > 5):
         ...
         try:
               x.execute("INSERT INTO eventsLog (UnitId, EventType, EventTime) VALUES (%s, %s, %s)", (id, 3,datetime.datetime.now()))
               conn.commit()
         except:
               conn.rollback()
   ...
   if(distance < 5 and trashStatus != "Empty"):
         ...
         try:
               ...
               conn.commit()
               x.execute("INSERT INTO eventsLog (UnitId, EventType, EventTime) VALUES (%s, %s, %s)", (id, 0,datetime.datetime.now()))
               conn.commit()
         except:
               conn.rollback()
   ...
   if(distance > 12 and trashStatus != "Full"):
         ...
         try:
               ...
               conn.commit()
               x.execute("INSERT INTO eventsLog (UnitId, EventType, EventTime) VALUES (%s, %s, %s)", (id, 1,datetime.datetime.now()))
               conn.commit()
